...
func (m *PromptManager) Read(ctx context.Context) (*Prompt, error) {
	p := new(Prompt)
	err := m.get(ctx, m.uri("prompts"), p)
	return p, err
}
...
func (m *PromptManager) Update(ctx context.Context, p *Prompt) error {
	return m.patch(ctx, m.uri("prompts"), p)
}
